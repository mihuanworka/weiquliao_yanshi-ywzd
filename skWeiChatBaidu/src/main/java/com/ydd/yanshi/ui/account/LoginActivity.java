...
import com.ydd.yanshi.bean.WXUploadResult;
...
import com.ydd.yanshi.helper.PasswordHelper;
...
import com.ydd.yanshi.wxapi.WXEntryActivity;
...
//        });
    ...
    private void initView() {
        ...
        if (!coreManager.getConfig().regeditPhoneOrName) {
...
*/
        ...
        if (!coreManager.getConfig().isOpenSMSCode) {
            // 在短信验证码关闭的的情况下隐藏短信短信登录
            mPhoneNumberEdit.setInputType(InputType.TYPE_CLASS_TEXT);
            mPhoneNumberEdit.setHint(R.string.username);
...
//                startActivityForResult(intent, SelectPrefixActivity.REQUEST_MOBILE_PREFIX_LOGIN);
    ...
    private void login(boolean third) {
        ...
        Log.e(HttpUtils.TAG, "登录网络请求参数：" + url);
