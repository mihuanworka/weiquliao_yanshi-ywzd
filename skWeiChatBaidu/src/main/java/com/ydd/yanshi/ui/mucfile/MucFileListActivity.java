...
import com.ydd.yanshi.view.LoadingDialog;
...
import static com.ydd.yanshi.ui.mucfile.DownManager.STATE_DOWNLOADING;
import static com.ydd.yanshi.ui.mucfile.DownManager.STATE_PAUSEDOWNLOAD;
...
public class MucFileListActivity extends BaseListActivity<MucFileListActivity.MucFileHolder> implements DownManager.DownLoadObserver {
    ...
    public void fillData(MucFileHolder holder, int position) {
        ...
        if (bean.getType() == 1) {
            // 图片直接显示
            Glide.with(MucFileListActivity.this)
                    .load(bean.getUrl())
                    .dontAnimate().skipMemoryCache(true) // 不使用内存缓存
                    .diskCacheStrategy(DiskCacheStrategy.NONE) // 不使用磁盘缓存
                    .override(100, 100).centerCrop().into(holder.ivInco);
        } else {
            // 加载本地
