...
public class ChatOverviewAdapter extends PagerAdapter {
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        if (!TextUtils.isEmpty(chatMessage.getFilePath()) && FileUtil.isExist(chatMessage.getFilePath())) {
            Glide.with(mContext).load(new File(chatMessage.getFilePath()))
                    .dontAnimate()
                    .skipMemoryCache(true)
                    .diskCacheStrategy(DiskCacheStrategy.NONE).into((ZoomImageView) view);
        } else {
            Glide.with(mContext).load(chatMessage.getContent())
                    .dontAnimate()
                    .skipMemoryCache(true)
                    .diskCacheStrategy(DiskCacheStrategy.NONE).into((ZoomImageView) view);
        }
