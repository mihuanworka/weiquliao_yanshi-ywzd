...
import com.ydd.yanshi.downloader.DownloadListener;
import com.ydd.yanshi.downloader.Downloader;
import com.ydd.yanshi.downloader.FailReason;
import com.ydd.yanshi.helper.AvatarHelper;
import com.ydd.yanshi.ui.message.ChatOverviewActivity;
import com.ydd.yanshi.ui.tool.SingleImagePreviewActivity;
import com.ydd.yanshi.util.FileUtil;
import com.ydd.yanshi.view.ChatImageView;
import com.ydd.yanshi.view.XuanProgressPar;
...
public class ImageViewHolder extends AChatHolderInterface {
    ...
    public void fillData(ChatMessage message) {
        ...
        if (FileUtil.isExist(filePath)) { // 本地存在
            //如果有本地数据，那么使用本地数据进行加载，避免下载数据
            //使用Glide进行异步加载
            Glide.with(mImageView.getContext()).load(filePath).error(R.drawable.fez)
...
//            }
            ...
            if (TextUtils.isEmpty(message.getContent())) {
                ...
                Glide.with(mImageView.getContext()).load(message.getContent()).error(R.drawable.fez)
                        .dontAnimate().skipMemoryCache(true)
