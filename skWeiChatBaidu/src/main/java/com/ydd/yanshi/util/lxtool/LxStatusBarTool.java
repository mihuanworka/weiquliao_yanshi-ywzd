...
/**
 * Created by liguoyue on 2017/12/7.
 */

public class LxStatusBarTool {
    ...
    public static void translucent(Activity activity, @ColorInt int colorOn5x) {
        ...
        //平板的情况
        if (LxDeviceTool.isTablet(activity)){
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
        ...
        // 小米和魅族4.4 以上版本支持沉浸式
        if (LxDeviceTool.isMeizu() || LxDeviceTool.isMIUI()) {
...
//            }
    ...
    private static boolean isMIUICustomStatusBarLightModeImpl() {
        return LxDeviceTool.isMIUIV5() || LxDeviceTool.isMIUIV6() ||
                LxDeviceTool.isMIUIV7() || LxDeviceTool.isMIUIV8();
    }
    ...
    public static boolean supportTransclentStatusBar6() {
        return !(LxDeviceTool.isZUKZ1() || LxDeviceTool.isZTKC2016());
    }
    ...
    private static void initStatusBarHeight(Context context) {
        ...
        if (LxDeviceTool.isTablet(context)
                && sStatusbarHeight > LxSmallTool.dip2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP)) {
            //状态栏高度大于25dp的平板，状态栏通常在下方
            sStatusbarHeight = 0;
        } else {
            if (sStatusbarHeight <= 0
                    || sStatusbarHeight > LxSmallTool.dip2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP * 2)) {
