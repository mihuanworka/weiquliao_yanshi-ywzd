...
public class NetWorkObservable extends Observable<NetWorkObserver> {
    ...
    NetWorkObservable(Context context) {
        ...
        LocalBroadcastManager.getInstance(mContext).registerReceiver(mNetWorkChangeReceiver, intentFilter);
        mRegisted = true;
    }
    ...
    public void release() {
        if (mRegisted && mContext != null) {
            LocalBroadcastManager.getInstance(mContext).unregisterReceiver(mNetWorkChangeReceiver);
            mRegisted = false;
        }
        unregisterAll();
    }
